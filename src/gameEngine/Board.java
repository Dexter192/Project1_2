...
public class Board {
	...
	/*
	 * public float getHeight(float x, float y) { Vector3 a = new Vector3(x,y,0);
	 * Terrain current = getTileOn(a); return current.getPosition().z; }
	 */
	public Terrain getTileOn(Vector3 position) {
		Terrain tileOn = null;
		for (Terrain t : terrainList) {
			if (t.getPosition().x <= position.x && t.getPosition().x + t.getWidth() >= position.x
					&& t.getPosition().y <= position.y && t.getPosition().y + t.getHeight() >= position.y) {
				tileOn = t;
